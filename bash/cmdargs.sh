...
case $1 in
    -h )
    echo 'You have entered "-h" which is for help.'
    ;;
    -v )
    echo 'You have entered "-v" which is for varbose.'
    ;;
    -d )
      case $2 in
        [1-5] )
        echo "You have entered -d for debug level number $2"
        shift
        ;;
        *)
        echo "Error message : The -d option must be followed with a number from 1 to 5"
        shift
      esac
    ;;
  *)
